...
## Voice-driven vehicle-trip entry

The assistant now recognizes Arabic add/insert vehicle-trip requests and collects the vehicle plate, loading location, unloading location, customer, cubic capacity, trip count, and notes in sequence. Every question is spoken and shown as text. The Android recognition request uses a 20-second minimum input window and 10-second silence allowance, with an **أحتاج وقتاً إضافياً** control that restarts listening without changing the active question. A confirmed draft creates a normalized `vehicle_trips` row linked to the vehicle, customer, authenticated user, conversation session, confirmation time, entry method, source transcript, and operational audit event. The APK was built by [GitHub Actions run 33036034937](https://github.com/Mozoo1999/ARQA-NEW/actions/runs/33036034937), has SHA-256 `9adcec5e487757ab4fe33f92b52cdb74dc3c30c414bb1ebeb70e7bee9e7a786d`, and is available at `/manus-storage/NARQA-EBOS-v0.1.0-vehicle-trip-voice-arm64_5fd642aa.apk`.
