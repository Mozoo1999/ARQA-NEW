...
function DocumentIntakeScreen({ mobileUser, onSignIn }: { mobileUser: MobileUser | null; onSignIn: () => void }) {
  const [imageUri, setImageUri] = useState<string | null>(null); const [sourceLabel, setSourceLabel] = useState(""); const [selectedFile, setSelectedFile] = useState<string | null>(null); const [rawText, setRawText] = useState(""); const [fields, setFields] = useState<DocumentFields>({ vendorName: "", amount: "", documentDate: "", taxNo: "", referenceNo: "" }); const [state, setState] = useState<"idle" | "processing" | "review">("idle"); const [error, setError] = useState<string | null>(null); const [reviewNote, setReviewNote] = useState<string | null>(null); const [aiAnalysis, setAiAnalysis] = useState<ProjectAiAnalysis | null>(null); const [operationalAnalysis, setOperationalAnalysis] = useState<OperationalAiAnalysis | null>(null); const [aiLoading, setAiLoading] = useState(false); const [submittedDraftId, setSubmittedDraftId] = useState<number | null>(null); const [submissionError, setSubmissionError] = useState<string | null>(null); const [submissionBusy, setSubmissionBusy] = useState(false);
