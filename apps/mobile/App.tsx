...
function CommandsScreen({ quickStartId, onQuickStartConsumed, mobileUser, onSignIn }: { quickStartId: number; onQuickStartConsumed: () => void; mobileUser: MobileUser | null; onSignIn: () => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
    ...
    {conversation ? <ConversationPanel conversation={conversation} answer={conversationAnswer} onChangeAnswer={setConversationAnswer} busy={conversationBusy} error={conversationError} outcome={conversationOutcome} onAnswer={() => void answerCurrentQuestion()} onConfirm={confirmCurrentConversation} onSpeak={() => { if (conversation.nextQuestion) Speech.speak(conversation.nextQuestion, { language: "ar-SA", rate: 0.9 }); }} /> : null}
  </ScrollView>;
}
