...
function CommandsScreen({ quickStartId, onQuickStartConsumed, mobileUser, onSignIn }: { quickStartId: number; onQuickStartConsumed: () => void; mobileUser: MobileUser | null; onSignIn: () => void }) {
  ...
  useSpeechRecognitionEvent("result", event => { const best = event.results[0]; if (!best?.transcript) return; setCommand(best.transcript); setVoiceConfidence(best.confidence >= 0 ? best.confidence : null); if (event.isFinal) { setIsListening(false); if (conversation?.nextQuestion) { setConversationAnswer(best.transcript); void submitConversationAnswer(best.transcript, "voice"); return; } if (voiceShortcutActive) { setVoiceShortcutActive(false); void beginConversation(best.transcript, "voice"); return; } Speech.speak("تم استلام النص. راجعه ثم اضغط تحليل الأمر.", { language: "ar-SA", rate: 0.9 }); } });
  ...
  const beginConversation = async (content: string, channel: ConversationChannel) => { if (!content.trim()) return; if (!mobileUser) { setConversationError("سجّل الدخول أولاً لبدء جلسة محفوظة مع المستخدم والتاريخ وطريقة الإدخال."); onSignIn(); return; } try { setConversationBusy(true); setConversationError(null); setConversationOutcome(null); const next = await startConversation(channel, content); setConversation(next); if (next.nextQuestion) Speech.speak(next.nextQuestion, { language: "ar-SA", rate: 0.9 }); else Speech.speak("اكتملت البيانات الأولية. راجع المسودة ثم أكد التنفيذ.", { language: "ar-SA", rate: 0.9 }); } catch (caught) { setConversationError(caught instanceof Error ? caught.message : "تعذر بدء جلسة المساعد."); } finally { setConversationBusy(false); } };
  const submitConversationAnswer = async (content = conversationAnswer, channel: ConversationChannel = "text") => { if (!conversation || !content.trim()) return; try { setConversationBusy(true); setConversationError(null); const next = await answerConversation(conversation.sessionId, channel, content); setConversation(next); setConversationAnswer(""); if (next.nextQuestion) Speech.speak(next.nextQuestion, { language: "ar-SA", rate: 0.9 }); else Speech.speak("المسودة مكتملة للمراجعة. اضغط تأكيد وتنفيذ عندما توافق.", { language: "ar-SA", rate: 0.9 }); } catch (caught) { setConversationError(caught instanceof Error ? caught.message : "تعذر حفظ الإجابة."); } finally { setConversationBusy(false); } };
    ...
    for (const asset of assets) {
      try {
        ...
        if (isPdf) {
          ...
          if (!text.trim()) { const rendered = await convertPdfToImages(asset.uri); const page = rendered.outputFiles?.[0]; if (!page) throw new Error("تعذر تحويل الصفحة الأولى إلى صورة"); const prepared = await manipulateAsync(page, [{ resize: { width: 2048 } }], { compress: 0.95, format: SaveFormat.JPEG }); if (!preview) preview = prepared.uri; text = (await ExpoTextExtractor.extractTextFromImage(prepared.uri)).join("\n"); }
        } else {
          const prepared = await manipulateAsync(asset.uri, [{ resize: { width: 2048 } }], { compress: 0.95, format: SaveFormat.JPEG }); if (!preview) preview = prepared.uri; text = (await ExpoTextExtractor.extractTextFromImage(prepared.uri)).join("\n");
        }
