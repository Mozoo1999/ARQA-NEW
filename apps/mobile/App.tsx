...
type Tab = "home" | "cost" | "commands" | "intake" | "invoice" | "sources" | "suppliers" | "customers" | "projects" | "reports" | "messages";
...
function MessageImportScreen({ mobileUser, onSignIn, onNavigate }: { mobileUser: MobileUser | null; onSignIn: () => void; onNavigate: (tab: Tab) => void }) {
  const [dashboard, setDashboard] = useState<MobileDashboard | null>(null); const [loading, setLoading] = useState(false); const [selectedContact, setSelectedContact] = useState<LiveRecord | null>(null); const [channel, setChannel] = useState<"manual_message" | "whatsapp" | "sms">("manual_message"); const [content, setContent] = useState(""); const [consent, setConsent] = useState(false); const [busy, setBusy] = useState(false); const [result, setResult] = useState<string | null>(null); const [error, setError] = useState<string | null>(null);
  useEffect(() => { if (!mobileUser) { setDashboard(null); return; } let active = true; setLoading(true); fetchMobileDashboard().then(data => { if (active) setDashboard(data); }).catch(caught => { if (active) setError(caught instanceof Error ? caught.message : "تعذر تحميل جهات الاتصال."); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [mobileUser?.id]);
  const createDraft = () => {
...
function ConversationPanel({ conversation, answer, onChangeAnswer, busy, error, outcome, onAnswer, onConfirm, onSpeak }: { conversation: ConversationProgress; answer: string; onChangeAnswer: (value: string) => void; busy: boolean; error: string | null; outcome: ConversationOutcome | null; onAnswer: () => void; onConfirm: () => void; onSpeak: () => void }) {
  return <View style={styles.resultCard}>
    ...
    {conversation.readyForReview && !outcome ? <View style={styles.reviewDivider}><Text style={styles.reviewWarning}>راجع جميع الحقول أعلاه. لن ينفذ النظام أي إنشاء أو إدراج قبل تأكيدك.</Text><PrimaryButton label={busy ? "يجري التنفيذ…" : "تأكيد وتنفيذ المسودة"} onPress={onConfirm} /></View> : null}
    {outcome ? <View style={styles.qualityCard}><Text style={styles.qualityTitle}>تم تنفيذ الموافقة</Text><Text style={styles.qualityText}>تم إنشاء {outcome.entityType} بالرقم #{outcome.entityId} وبالحالة {outcome.status}، مع حفظ المستخدم والتاريخ وطريقة الإدخال وسجل المحادثة.</Text></View> : null}
...
function CommandsScreen({ quickStartId, onQuickStartConsumed, mobileUser, onSignIn }: { quickStartId: number; onQuickStartConsumed: () => void; mobileUser: MobileUser | null; onSignIn: () => void }) {
  ...
  const startListening = async () => {
    ...
    setIsListening(true); ExpoSpeechRecognitionModule.start({ lang: "ar-SA", interimResults: true, maxAlternatives: 1, continuous: false, addsPunctuation: true, contextualStrings: ["NARQA", "EBOS", "إضافة نقلة سيارة", "رقم السيارة", "مكان الحمولة", "مكان التفريغ", "اسم العميل", "تكعيب السيارة", "عدد النقلات", "ملاحظات", "فاتورة", "مورد", "إذن استلام", "مصروف", "إيراد", "موافق", "إلغاء"], androidIntentOptions: { EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS: 10000, EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS: 10000, EXTRA_SPEECH_INPUT_MINIMUM_LENGTH_MILLIS: 20000 } });
  };
  const extendListeningTime = () => { ExpoSpeechRecognitionModule.abort(); setIsListening(false); setTimeout(() => { void startListening(); }, 300); };
  const speakAndListen = (text: string) => { Speech.stop(); Speech.speak(text, { language: "ar-SA", rate: 0.9, onDone: () => { void startListening(); } }); };
  ...
  const beginConversation = async (content: string, channel: ConversationChannel) => { if (!content.trim()) return; if (!mobileUser) { setConversationError("سجّل الدخول أولاً لبدء جلسة محفوظة مع المستخدم والتاريخ وطريقة الإدخال."); onSignIn(); return; } try { setConversationBusy(true); setConversationError(null); setConversationOutcome(null); const next = await startConversation(channel, content); setConversation(next); if (next.nextQuestion) speakAndListen(next.nextQuestion); else speakAndListen("اكتملت البيانات الأولية. راجع ملخص المسودة. هل توافق على التنفيذ؟ قل موافق للتنفيذ أو إلغاء للإيقاف."); } catch (caught) { setConversationError(caught instanceof Error ? caught.message : "تعذر بدء جلسة المساعد."); } finally { setConversationBusy(false); } };
  const submitConversationAnswer = async (content = conversationAnswer, channel: ConversationChannel = "text") => { if (!conversation || !content.trim()) return; try { setConversationBusy(true); setConversationError(null); const next = await answerConversation(conversation.sessionId, channel, content); setConversation(next); setConversationAnswer(""); if (next.nextQuestion) speakAndListen(next.nextQuestion); else speakAndListen("المسودة مكتملة للمراجعة. هل توافق على التنفيذ؟ قل موافق للتنفيذ أو إلغاء للإيقاف."); } catch (caught) { setConversationError(caught instanceof Error ? caught.message : "تعذر حفظ الإجابة."); } finally { setConversationBusy(false); } };
  ...
  useEffect(() => { if (!quickStartId) return; setVoiceShortcutActive(true); void startListening(); onQuickStartConsumed(); }, [quickStartId]);
  return <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
    ...
    {conversation ? <ConversationPanel conversation={conversation} answer={conversationAnswer} onChangeAnswer={setConversationAnswer} busy={conversationBusy} error={conversationError} outcome={conversationOutcome} onAnswer={() => void submitConversationAnswer()} onConfirm={confirmCurrentConversation} onSpeak={() => { if (conversation.nextQuestion) Speech.speak(conversation.nextQuestion, { language: "ar-SA", rate: 0.9 }); }} /> : null}
  </ScrollView>;
}
...
function DocumentIntakeScreen({ mobileUser, onSignIn }: { mobileUser: MobileUser | null; onSignIn: () => void }) {
  ...
  const analyze = async (uri: string, label: string, sourceType: "camera" | "image") => { setState("processing"); setError(null); setReviewNote(null); setAiAnalysis(null); setSubmittedDraftId(null); setSubmissionError(null); try { const result = await analyzeImage(uri, sourceType, label); setImageUri(result.preparedUri); setSourceLabel(label); setRawText(result.text); setFields(result.fields); setAiAnalysis(result.visual); setReviewNote(result.note); setState("review"); } catch (caught) { setState("idle"); setError(caught instanceof Error ? caught.message : "تعذر تحليل الصورة. سجّل الدخول ثم أعد المحاولة."); } };
  ...
  const chooseDocument = async () => {
    ...
    setState("processing"); setSelectedFile(assets[0]?.name ?? null); setSelectedFiles(assets.map(asset => asset.name)); setFileResults([]); setAiAnalysis(null); setOperationalAnalysis(null); setSubmittedDraftId(null); setSubmissionError(null);
    ...
    for (const asset of assets) {
      try {
        ...
        if (isPdf) {
          if (!isPdfTextExtractorAvailable()) throw new Error("محلل PDF غير متاح في هذا الإصدار");
          if (await isPasswordProtected(asset.uri)) throw new Error("PDF محمي بكلمة مرور");
          text = await extractPdfText(asset.uri);
          if (!text.trim()) { const rendered = await convertPdfToImages(asset.uri); const page = rendered.outputFiles?.[0]; if (!page) throw new Error("تعذر تحويل الصفحة الأولى إلى صورة"); if (!preview) preview = page; text = (await analyzeImage(page, "image", asset.name)).text; }
        } else {
