...
import { extractDocumentFields, type DocumentFields } from "./src/document-intake";
...
function HomeScreen({ onNavigate }: { onNavigate: (tab: Tab) => void }) {
  return <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
    <SectionTitle eyebrow="ARQA / SUPPLY CORE" title="مركز التشغيل المحمول" description="وصول سريع إلى محرك التكلفة، الأوامر العربية، وتحليل المستندات على Android وiOS والأجهزة اللوحية." />
    <View style={styles.statusBanner}><View style={styles.statusDot} /><View style={styles.statusCopy}><Text style={styles.statusTitle}>المحرك المحلي جاهز</Text><Text style={styles.statusText}>الحسابات والتحليل النصي يعملان من التطبيق، ولا ينفذ أي إدخال مالي تلقائياً دون مراجعة.</Text></View></View>
    <View style={styles.cardGrid}>
      <View style={styles.metricCard}><Text style={styles.metricLabel}>المحرك</Text><Text style={styles.metricValue}>Cost Chain</Text><Text style={styles.metricHint}>شراء · نقل · هالك · إدارة · ربح</Text></View>
      <View style={styles.metricCard}><Text style={styles.metricLabel}>الأوامر</Text><Text style={styles.metricValue}>صوت ونص</Text><Text style={styles.metricHint}>إذن ميكروفون ثم تحويل عربي إلى نص للمراجعة</Text></View>
      <View style={styles.metricCard}><Text style={styles.metricLabel}>المستندات</Text><Text style={styles.metricValue}>OCR محلي</Text><Text style={styles.metricHint}>تحسين صورة ثم استخراج قابل للتصحيح</Text></View>
    </View>
    <View style={styles.actionCard}><View style={styles.actionCopy}><Text style={styles.actionTitle}>ابدأ بحساب عرض سعر شفاف</Text><Text style={styles.actionText}>أدخل الكمية ومكونات التكلفة لعرض تكلفة الوحدة والسعر المقترح وكل سطر حسابي.</Text></View><PrimaryButton label="فتح محرك التكلفة" onPress={() => onNavigate("cost")} /></View>
    <View style={styles.actionCard}><View style={styles.actionCopy}><Text style={styles.actionTitle}>تحدث بأمر عربي</Text><Text style={styles.actionText}>اضغط الميكروفون، راجع النص الناتج، ثم حلّل النية قبل فتح مسار المراجعة.</Text></View><SecondaryButton label="فتح الأوامر الصوتية" onPress={() => onNavigate("commands")} /></View>
    <View style={styles.actionCard}><View style={styles.actionCopy}><Text style={styles.actionTitle}>حلّل إيصالاً أو مستنداً</Text><Text style={styles.actionText}>التقط صورة واضحة أو اخترها من الجهاز، ثم صحح الحقول المستخرجة قبل الاعتماد.</Text></View><SecondaryButton label="فتح تحليل المستندات" onPress={() => onNavigate("intake")} /></View>
    <View style={styles.infoCard}><Text style={styles.infoTitle}>وحدات EBOS الفعلية</Text><Text style={styles.infoText}>تفتح الوحدات التالية المسارات المنشورة للنظام وتستخدم جلسة الويب الحالية.</Text><View style={styles.moduleLinks}>{operationalModules.map(module => <Pressable key={module.path} onPress={() => openOperationalModule(module.path)} style={({ pressed }) => [styles.moduleLink, pressed && styles.pressed]}><Text style={styles.moduleLinkText}>{module.label}</Text><Text style={styles.moduleLinkArrow}>←</Text></Pressable>)}</View></View>
  </ScrollView>;
}
...
function DocumentIntakeScreen() {
  const [imageUri, setImageUri] = useState<string | null>(null); const [sourceLabel, setSourceLabel] = useState(""); const [rawText, setRawText] = useState(""); const [fields, setFields] = useState<DocumentFields>({ vendorName: "", amount: "", documentDate: "", taxNo: "", referenceNo: "" }); const [state, setState] = useState<"idle" | "processing" | "review">("idle"); const [error, setError] = useState<string | null>(null);
  ...
  return <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
    <SectionTitle eyebrow="DOCUMENT INTELLIGENCE" title="تحليل الصور والمستندات" description="التقط أو اختر صورة واضحة؛ يحسن التطبيق أبعادها ثم يستخرج النص محلياً ويعرض كل حقل كمسودة قابلة للتصحيح قبل أي اعتماد." />
...
const styles = StyleSheet.create({
  app: { flex: 1, backgroundColor: colors.background }, authButton: { paddingHorizontal: 11, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: colors.accent }, authButtonText: { color: colors.accent, fontSize: 11, fontWeight: "800" }, topBar: { minHeight: 78, paddingHorizontal: 20, paddingTop: 18, paddingBottom: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: colors.border }, brand: { color: colors.accent, fontSize: 23, fontWeight: "800", letterSpacing: 2 }, brandSubtitle: { color: colors.muted, fontSize: 11, marginTop: 2, letterSpacing: 1 }, deviceBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20, backgroundColor: colors.panelRaised, borderWidth: 1, borderColor: colors.border }, deviceBadgeText: { color: colors.muted, fontSize: 10, fontWeight: "700", letterSpacing: 0.6 }, body: { flex: 1 }, bodyTablet: { flexDirection: "row" }, sideNav: { width: 235, padding: 18, borderRightWidth: 1, borderRightColor: colors.border, backgroundColor: "#0D1728" }, navHeading: { color: colors.muted, fontSize: 12, marginBottom: 14, textAlign: "right" }, navItem: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 13, paddingVertical: 13, borderRadius: 12, marginBottom: 7 }, navItemActive: { backgroundColor: colors.accentSoft, borderWidth: 1, borderColor: "#71552D" }, navIcon: { color: colors.muted, fontSize: 20, width: 25, textAlign: "center" }, navText: { color: colors.muted, fontSize: 14, flex: 1, textAlign: "right" }, navTextActive: { color: colors.accent, fontWeight: "700" }, navFooter: { marginTop: "auto", paddingTop: 16, borderTopWidth: 1, borderTopColor: colors.border }, navFooterTitle: { color: colors.text, fontSize: 12, textAlign: "right", marginBottom: 4 }, navFooterText: { color: colors.muted, fontSize: 10, textAlign: "right" }, mainContent: { flex: 1 }, scrollContent: { padding: 20, paddingBottom: 40, maxWidth: 1100, width: "100%", alignSelf: "center" }, sectionHeader: { marginBottom: 22 }, eyebrow: { color: colors.accent, fontSize: 11, letterSpacing: 1.4, fontWeight: "700", textAlign: "right", marginBottom: 7 }, title: { color: colors.text, fontSize: 30, lineHeight: 38, fontWeight: "800", textAlign: "right" }, description: { color: colors.muted, fontSize: 14, lineHeight: 23, textAlign: "right", marginTop: 8 }, statusBanner: { flexDirection: "row", alignItems: "center", padding: 16, borderRadius: 15, backgroundColor: "#102A23", borderWidth: 1, borderColor: "#235D4A", marginBottom: 16 }, statusDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: colors.success, marginRight: 12 }, statusCopy: { flex: 1 }, statusTitle: { color: colors.success, fontSize: 14, fontWeight: "800", textAlign: "right" }, statusText: { color: "#B1D7C5", fontSize: 12, lineHeight: 19, textAlign: "right", marginTop: 3 }, cardGrid: { gap: 12, marginBottom: 16 }, metricCard: { backgroundColor: colors.panel, borderRadius: 15, borderWidth: 1, borderColor: colors.border, padding: 16 }, metricLabel: { color: colors.muted, fontSize: 12, textAlign: "right" }, metricValue: { color: colors.text, fontSize: 20, fontWeight: "800", textAlign: "right", marginTop: 7 }, metricHint: { color: colors.muted, fontSize: 11, textAlign: "right", marginTop: 5 }, actionCard: { backgroundColor: colors.panel, borderRadius: 15, borderWidth: 1, borderColor: colors.border, padding: 18, marginBottom: 12 }, actionCopy: { marginBottom: 15 }, actionTitle: { color: colors.text, fontSize: 16, fontWeight: "800", textAlign: "right" }, actionText: { color: colors.muted, fontSize: 13, lineHeight: 21, textAlign: "right", marginTop: 6 }, primaryButton: { backgroundColor: colors.accent, minHeight: 46, borderRadius: 11, alignItems: "center", justifyContent: "center", paddingHorizontal: 18 }, primaryButtonText: { color: "#1A130A", fontSize: 14, fontWeight: "800" }, secondaryButton: { backgroundColor: "transparent", minHeight: 46, borderRadius: 11, alignItems: "center", justifyContent: "center", paddingHorizontal: 18, borderWidth: 1, borderColor: colors.accent }, secondaryButtonText: { color: colors.accent, fontSize: 14, fontWeight: "800" }, pressed: { opacity: 0.75, transform: [{ scale: 0.98 }] }, formCard: { backgroundColor: colors.panel, borderRadius: 15, borderWidth: 1, borderColor: colors.border, padding: 16, marginBottom: 16 }, formGrid: { gap: 13 }, fieldGroup: { marginBottom: 10 }, fieldLabel: { color: colors.muted, fontSize: 12, textAlign: "right", marginBottom: 7 }, inputRow: { flexDirection: "row", alignItems: "center", backgroundColor: colors.background, borderRadius: 10, borderWidth: 1, borderColor: colors.border, minHeight: 46 }, input: { flex: 1, color: colors.text, fontSize: 15, paddingHorizontal: 12, paddingVertical: 10, minHeight: 44 }, suffix: { color: colors.muted, fontSize: 11, paddingRight: 12 }, resultCard: { backgroundColor: colors.panelRaised, borderRadius: 15, borderWidth: 1, borderColor: "#72572F", padding: 18, marginBottom: 16 }, resultHero: { borderBottomWidth: 1, borderBottomColor: colors.border, paddingBottom: 16, marginBottom: 14 }, resultLabel: { color: colors.muted, fontSize: 12, textAlign: "right" }, resultValue: { color: colors.accent, fontSize: 31, fontWeight: "900", textAlign: "right", marginTop: 6 }, resultSub: { color: colors.text, fontSize: 13, textAlign: "right", marginTop: 4 }, resultMetaRow: { flexDirection: "row", gap: 8, marginBottom: 17 }, resultMeta: { flex: 1, backgroundColor: colors.panel, borderRadius: 10, padding: 10, minHeight: 60 }, resultMetaLabel: { color: colors.muted, fontSize: 10, textAlign: "right" }, resultMetaValue: { color: colors.text, fontSize: 13, fontWeight: "700", textAlign: "right", marginTop: 5 }, linesHeading: { color: colors.text, fontSize: 14, fontWeight: "800", textAlign: "right", marginBottom: 8 }, lineRow: { flexDirection: "row", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: colors.border, paddingVertical: 10 }, lineLabel: { color: colors.muted, fontSize: 12 }, lineAmount: { color: colors.text, fontSize: 12, fontWeight: "700" }, errorCard: { backgroundColor: "#351D24", borderRadius: 15, borderWidth: 1, borderColor: "#743D48", padding: 16, marginBottom: 16 }, errorTitle: { color: colors.danger, fontSize: 14, fontWeight: "800", textAlign: "right" }, errorText: { color: "#F2B8BC", fontSize: 12, textAlign: "right", marginTop: 6, lineHeight: 19 }, commandInput: { backgroundColor: colors.background, borderRadius: 10, borderWidth: 1, borderColor: colors.border, minHeight: 120, marginBottom: 15 }, inlineError: { color: "#F2B8BC", fontSize: 12, lineHeight: 19, textAlign: "right", marginTop: 12 }, hintText: { color: colors.muted, fontSize: 12, lineHeight: 20, textAlign: "right", marginTop: 9, marginBottom: 12 }, infoCard: { backgroundColor: colors.panel, borderRadius: 15, borderWidth: 1, borderColor: colors.border, padding: 16, marginBottom: 16 }, infoTitle: { color: colors.accent, fontSize: 14, fontWeight: "800", textAlign: "right" }, infoText: { color: colors.muted, fontSize: 12, lineHeight: 20, textAlign: "right", marginTop: 6 }, parsedSummary: { color: colors.text, fontSize: 17, lineHeight: 26, fontWeight: "700", textAlign: "right", marginTop: 8, marginBottom: 15 }, reasonText: { color: colors.muted, fontSize: 12, lineHeight: 20, textAlign: "right", marginTop: 5 }, reviewDivider: { marginTop: 16, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 14, gap: 10 }, reviewWarning: { color: colors.accent, fontSize: 12, textAlign: "right", fontWeight: "700" }, moduleLinks: { marginTop: 12, gap: 8 }, moduleLink: { flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between", backgroundColor: colors.background, borderRadius: 10, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 12, paddingVertical: 11 }, moduleLinkText: { color: colors.text, fontSize: 12, fontWeight: "700", textAlign: "right" }, moduleLinkArrow: { color: colors.accent, fontSize: 16 }, buttonRow: { flexDirection: "row", gap: 10 }, flexButton: { flex: 1 }, stack: { gap: 10 }, documentPreview: { width: "100%", height: 320, backgroundColor: "#07111F", borderRadius: 15, borderWidth: 1, borderColor: colors.border, marginBottom: 16 }, bottomNav: { flexDirection: "row", backgroundColor: "#0D1728", borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 8, paddingBottom: 10 }, bottomNavItem: { flex: 1, alignItems: "center", justifyContent: "center", minHeight: 52 }, bottomNavText: { color: colors.muted, fontSize: 10, marginTop: 3 },
});
